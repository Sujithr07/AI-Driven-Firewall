"""
mlops/tracking.py — Central MLflow experiment tracking.
...
so the rest of the codebase never needs to guard for ImportError.
"""
...
# ── Tracking URI (SQLite by default, overridable via env) ─────────────────────

_BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
_DB_PATH  = os.path.join(_BASE_DIR, "data", "mlflow.db").replace("\\", "/")
_DEFAULT_URI = f"sqlite:///{_DB_PATH}"
TRACKING_URI = os.getenv("MLFLOW_TRACKING_URI", _DEFAULT_URI)
...
def log_confusion_matrix(cm: list, labels: list[str],
                          name: str = "confusion_matrix") -> None:
    """Render a confusion-matrix heatmap and upload it as an MLflow artifact."""
...
def log_feature_importance(model: Any, feature_names: list[str],
                            name: str = "feature_importance") -> None:
    """Render a feature-importance bar chart and upload it as an MLflow artifact."""
...
def log_artifact(path: str, artifact_path: str | None = None) -> None:
    """Upload a file as an MLflow artifact (no-op if file does not exist)."""
...
class RLTracker:
    ...
    The run is created eagerly (then immediately ended) so the run_id is
    stable.  Each log_step / log_hyperparams call resumes the run, appends
    data, then ends it again — the standard MLflow pattern for long-running
    online processes.

    All methods are silent no-ops when MLflow is unavailable.
    """
