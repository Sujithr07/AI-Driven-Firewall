"""Data models, hash-chain helpers, and admin-user seeding."""
...
from app.db.client import supabase, DB_PATH
...
def _get_latest_hash(table: str) -> str:
    ...
    try:
        ...
        row = cur.fetchone()
        return row[0] if row and row[0] is not None else 'GENESIS'
    finally:
        conn.close()
...
def init_db():
    ...
    if not admin_pass:
        ...
        print('[Init] Generated admin password and saved to .env')
