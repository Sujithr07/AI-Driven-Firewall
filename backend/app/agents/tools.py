...
def get_threat_stats(hours: int = 1) -> str:
    ...
    sec_rows = _db("SELECT decision, severity, source_ip FROM security_logs WHERE timestamp >= ?", (since_ts,))
    det_rows = _db("SELECT rl_action, severity, src_ip, is_malicious FROM detection_logs WHERE timestamp >= ?", (since_ts,))
    ...
    return "\n".join([f"Period: last {hours}h", f"Security log events: {len(sec_rows)}",
                      f"Detection events: {len(det_rows)} ({malicious} malicious)",
                      f"Decisions: {decision_counts}", f"Severities: {severity_counts}",
...
def check_ip_reputation(ip_address: str) -> str:
    """Check an IP address's security history: block counts, decisions, and current block status."""
    logs = _db("SELECT decision, severity, reason, timestamp FROM security_logs WHERE source_ip = ? ORDER BY timestamp DESC LIMIT 20", (ip_address,))
    det_logs = _db("SELECT rl_action, severity, reason, timestamp FROM detection_logs WHERE src_ip = ? ORDER BY timestamp DESC LIMIT 10", (ip_address,))
    responses = _db("SELECT rule_type, reason, reversed, created_at FROM response_actions WHERE src_ip = ? ORDER BY timestamp DESC LIMIT 5", (ip_address,))
    ...
    parts = [f"IP: {ip_address}", f"Security log entries: {len(logs)} | Decisions: {decision_counts}",
             f"Detection events: {len(det_logs)}", f"Response actions taken: {len(responses)}",
...
def suggest_action(threat_type: str, severity: str, src_ip: str) -> str:
    """Suggest the appropriate firewall response for a threat."""
    sev = severity.lower(); ttype = threat_type.lower()
    if sev == "critical":
        action = "BLOCK"
