...
def train_and_evaluate_nslkdd():
    ...
    legacy_path = os.path.join(_DATA_DIR, 'eval_results_nslkdd.json')
...
def main():
    ...
    from app.core.config import DATA_DIR as _D
    os.makedirs(_D, exist_ok=True)
    _eval_path = os.path.join(_D, 'eval_results.json')
    with open(_eval_path, 'w') as f:
        json.dump(combined, f, indent=2)
    print(f"\nCombined results saved -> {_eval_path}")

    if MLFLOW_AVAILABLE:
        try:
            mlflow.set_experiment(EXPERIMENT_EVALUATION)
            with mlflow.start_run(run_name="combined-results-artifact"):
                log_artifact(_eval_path, artifact_path="results")
