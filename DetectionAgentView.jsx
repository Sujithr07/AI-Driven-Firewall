...
const DetectionAgentView = ({ token, onNavigateToXAI }) => {
    ...
    return (
        <div className="p-8">
            ...
            {/* Local Machine IPs Info */}
            {isRunning && agentStatus?.local_ips && agentStatus.local_ips.length > 0 && (
                <div className="bg-[#161b22] p-3 rounded-xl border border-gray-800 mb-4">
                    <p className="text-xs text-gray-500 uppercase tracking-wider mb-1">Local Machine IPs (Your Traffic)</p>
                    <div className="flex flex-wrap gap-2">
                        {agentStatus.local_ips.map((ip, idx) => (
                            <span key={idx} className="px-2 py-1 bg-blue-900/30 text-blue-300 text-xs font-mono rounded">
                                {ip}
                            </span>
                        ))}
                    </div>
                    <p className="text-xs text-gray-500 mt-2">
                        In the detection table: <span className="text-blue-300">Source IPs matching these are OUTBOUND traffic (you connecting to others)</span>. 
                        <span className="text-green-300">External IPs as Source are INBOUND traffic (others connecting to you)</span>.
                    </p>
                </div>
            )}
